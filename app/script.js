...
// ===== HEX INPUT (auto-format, no letters except a-f) =====
function onHexInput(input, errId) {
  let v = input.value;
  // Remove anything not hex
  v = v.replace(/[^0-9a-fA-F\s]/g, '');
  // Remove extra spaces
  v = v.replace(/\s+/g, ' ');
  // Auto-insert space after every 2 hex chars
  let raw = v.replace(/\s/g, '');
  if (raw.length > 8) raw = raw.slice(0, 8);
  let formatted = raw.match(/.{1,2}/g)?.join(' ') || '';
  // Only reformat if cursor is at end (avoid cursor jumping mid-edit)
...
function validateInputs() {
  ...
  document.getElementById('addrErr').textContent = va ? '' : 'Must be 4 hex bytes e.g. 20 00 00 00';
  document.getElementById('cmdErr').textContent = vc ? '' : 'Must be 4 hex bytes e.g. 09 00 00 00';
...
function toggleSaveForm() {
  ...
  if (f.classList.contains('open')) { refreshRemoteSelect(); document.getElementById('saveNameInput').focus(); }
}
...
function onSaveRemoteChange() {
  ...
  const nrf = document.getElementById('saveNewRemoteFields');
  nrf.style.display = v === '__new__' ? 'block' : 'none';
...
function parseIRFile(text) {
  ...
  for (let raw of lines) {
    ...
    if (l.startsWith('name:')) {
      if (cur) { if (cur.addr && cur.cmd) buttons.push(cur); }
  ...
  // Only keep NEC-like (parsed type with addr+cmd), skip raw/unknown
...
function importIRFiles(files, folderId=null, inputEl=null) {
  ...
  Array.from(files).forEach(f => {
    ...
    reader.onload = e => {
      ...
      const buttons = parseIRFile(e.target.result);
  ...
  fileList.forEach(f => {
    ...
    for (let i = 0; i < parts.length-1; i++) {
      ...
      if (!folderMap[path]) {
        ...
        const fid = genId();
        folderMap[path] = fid;
        library.push({ type:'folder', id:fid, name:parts[i], parentId, open:true });
      }
    }
  });
  fileList.forEach(f => {
...
function renderTreeLevel(container, parentId, depth) {
  ...
  folders.forEach(folder => {
    ...
    row.innerHTML = `<i class="ti ti-chevron-right ${isOpen?'open':''}"></i><i class="ti ${isOpen?'ti-folder-open':'ti-folder'}"></i><span class="tree-label">${escHtml(folder.name)}</span>
    ...
    row.addEventListener('click', e => { if(e.target.closest('.tree-actions')) return; folder.open = !folder.open; saveLib(); renderSidebarTree(); });
  ...
  remotes.forEach(remote => {
    ...
    row.innerHTML = `<i class="ti ti-device-remote-control"></i><span class="tree-label">${escHtml(remote.name)}</span>
      <span class="tree-actions">
        <button class="tree-act" title="Rename" onclick="openRenameModal(event,'remote',${remote.id})"><i class="ti ti-pencil"></i></button>
        <button class="tree-act" title="Move" onclick="openMoveModal(event,${remote.id})"><i class="ti ti-arrows-move"></i></button>
        <button class="tree-act del" title="Delete" onclick="openDeleteModal(event,'remote',${remote.id})"><i class="ti ti-trash"></i></button>
      </span>`;
    row.addEventListener('click', e => { if(e.target.closest('.tree-actions')) return; selectedRemoteId=remote.id; renderSidebarTree(); switchTab('library'); });
    container.appendChild(row);
  });
}
...
function renderLibraryOverview(el) {
  el.innerHTML = '';
  if (!library.length) {
    el.innerHTML = `<div class="empty-state"><i class="ti ti-satellite"></i><p>No remotes yet. Import .ir files or save manual signals.</p></div>`;
    return;
  }
  const hdr = document.createElement('div');
  hdr.className = 'section-hdr';
  hdr.innerHTML = `<span class="section-title">My Library</span>`;
  el.appendChild(hdr);
  // Root folders
...
function renderFolderSection(container, folder) {
  ...
  const isOpen = folder.open !== false;
  hdr.innerHTML = `<i class="ti ${isOpen?'ti-chevron-down':'ti-chevron-right'}" style="font-size:12px;color:var(--text3);"></i><i class="ti ti-folder" style="color:var(--text3);font-size:14px;"></i><span class="folder-section-name">${escHtml(folder.name)}</span>`;
...
function appendRemoteCard(container, remote) {
  ...
  card.innerHTML = `
    <div class="remote-card-hdr">
      <i class="ti ti-device-remote-control" style="color:var(--accent2);font-size:16px;"></i>
    ...
    <div class="chip-row">
      ${remote.buttons.slice(0,8).map(b=>`<span class="signal-chip">${escHtml(b.name)}</span>`).join('')}
      ${remote.buttons.length>8 ? `<span class="signal-chip" style="color:var(--text3);">+${remote.buttons.length-8} more</span>` : ''}
    </div>`;
...
function buildSignalBox(btn, remoteId) {
  ...
  box.innerHTML = `
    <div class="signal-box-header">
      ...
      <button class="btn btn-ghost btn-sm btn-icon btn-danger-ghost" title="Delete" onclick="deleteButton(${btn.id},${remoteId})" style="color:var(--text3);"><i class="ti ti-trash"></i></button>
    </div>
    <div id="sname-edit-${btn.id}" style="display:none;margin-bottom:10px;">
      <div style="display:flex;gap:6px;align-items:center;">
        <input type="text" id="sname-inp-${btn.id}" value="${escAttr(btn.name)}" style="width:200px;" oninput="validateSigName(${btn.id})">
    ...
    <div class="signal-meta">
      <span class="signal-chip addr"><i class="ti ti-map-pin" style="font-size:10px;vertical-align:-1px;"></i> ${escHtml(btn.addr||'—')}</span>
      <span class="signal-chip cmd"><i class="ti ti-terminal" style="font-size:10px;vertical-align:-1px;"></i> ${escHtml(btn.cmd||'—')}</span>
    ...
    <div id="sdesc-edit-${btn.id}" style="display:none;margin-bottom:10px;">
...
function validateAddBtn(remoteId) {
  ...
  document.getElementById('abn-name-err-'+remoteId).textContent = vn ? '' : (name==='' ? '' : 'Name required');
...
function confirmAddButton(remoteId) {
  ...
  const emp = document.getElementById('remote-empty-'+remoteId);
  if (emp) emp.remove();
  // Remove add box
  document.getElementById('addBtnBoxWrap-'+remoteId).innerHTML = '';
  // Append new signal box before end
  const el = document.getElementById('libraryContent');
  el.appendChild(buildSignalBox(newBtn, remoteId));
  appendLog(`Added button "${name}" to "${remote.name}".`, 'sys');
}
...
function confirmEditName(btnId, remoteId) {
  ...
  const v = document.getElementById('sname-inp-'+btnId).value.trim();
  const remote = library.find(r=>r.id===remoteId);
  remote.buttons.find(b=>b.id===btnId).name = v;
  saveLib();
...
function confirmEditDesc(btnId, remoteId) {
  ...
  const remote = library.find(r=>r.id===remoteId);
  remote.buttons.find(b=>b.id===btnId).desc = v;
  saveLib();
...
function deleteButton(btnId, remoteId) {
  ...
  const remote = library.find(r=>r.id===remoteId);
  remote.buttons = remote.buttons.filter(b=>b.id!==btnId);
  saveLib();
  document.getElementById('sigbox-'+btnId)?.remove();
}
function sendButtonSignal(addr, cmd) {
  if (!writer) { appendLog('Not connected.','err'); switchTab('transmit'); return; }
...
function confirmFolder() {
  ...
  library.push({ type:'folder', id:genId(), name:document.getElementById('folderNameInput').value.trim(), parentId:_folderParentId, open:true });
...
let _moveRemoteId;
function openMoveModal(e, remoteId) {
  if(e) e.stopPropagation();
  _moveRemoteId=remoteId;
  refreshFolderSelect('moveSelect');
  document.getElementById('moveModal').classList.add('open');
}
function closeMoveModal() { document.getElementById('moveModal').classList.remove('open'); }
function confirmMove() {
  const fid = document.getElementById('moveSelect').value;
  library.find(r=>r.id===_moveRemoteId).folderId = fid ? parseInt(fid) : null;
  saveLib(); closeMoveModal(); renderSidebarTree();
  if (currentTab==='library') renderLibraryPane();
}
...
['folderModal','renameModal','deleteModal','moveModal'].forEach(id => {
  document.getElementById(id).addEventListener('click', function(e){ if(e.target===this) this.classList.remove('open'); });
});

// ===== IRDB =====
let irdbData=null, irdbExpanded={}, irdbSelectedPath=null;

async function initIRDB() {
  if (irdbData) { renderIRDBTree(); return; }
  document.getElementById('irdbTree').innerHTML = `<div style="display:flex;align-items:center;gap:8px;padding:12px;color:var(--text3);font-family:var(--mono);font-size:12px;"><span class="spinner"></span> Loading...</div>`;
  document.getElementById('irdbRemoteView').innerHTML = '';
  try {
...
function renderIRDBLevel(container, node, path, depth, search) {
  ...
  keys.forEach(key => {
    ...
    const row=document.createElement('div');
    row.style.cssText=`padding:5px ${12}px 5px ${10+depth*12}px;display:flex;align-items:center;gap:5px;cursor:pointer;color:var(--text2);font-family:var(--mono);font-size:12px;transition:background 0.1s;`;
    row.innerHTML=`<i class="ti ${isOpen?'ti-chevron-down':'ti-chevron-right'}" style="font-size:10px;color:var(--text3);width:12px;"></i><i class="ti ${isOpen?'ti-folder-open':'ti-folder'}" style="font-size:13px;"></i><span>${escHtml(key)}</span>`;
    row.onmouseenter=()=>row.style.background='var(--bg3)';
    row.onmouseleave=()=>row.style.background='';
    row.onclick=()=>{ irdbExpanded[cp]=!irdbExpanded[cp]; renderIRDBTree(); };
    container.appendChild(row);
  ...
  files.forEach(f => {
    ...
    const row=document.createElement('div');
    row.style.cssText=`padding:5px 12px 5px ${10+depth*12+12}px;display:flex;align-items:center;gap:5px;cursor:pointer;font-family:var(--mono);font-size:12px;transition:background 0.1s;${isSel?'background:var(--accent-dim);color:var(--accent2);':'color:var(--text2);'}`;
    row.innerHTML=`<i class="ti ti-device-remote-control" style="font-size:13px;"></i><span>${escHtml(f.name.replace('.ir',''))}</span>`;
...
async function loadIRDBFile(file) {
  ...
  el.innerHTML=`<div style="display:flex;align-items:center;gap:8px;padding:20px;color:var(--text3);font-family:var(--mono);font-size:12px;"><span class="spinner"></span> Loading...</div>`;
  try {
    ...
    el.innerHTML=`<div style="padding:20px;color:var(--red);font-family:var(--mono);font-size:12px;"><i class="ti ti-alert-circle"></i> Failed: ${escHtml(String(e))}</div>`;
  }
}
...
function renderIRDBRemote(el, file, buttons) {
  ...
  // Header
  const hdr=document.createElement('div');
  hdr.style.cssText='display:flex;align-items:center;gap:10px;margin-bottom:16px;padding:14px 16px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--r2);';
  hdr.innerHTML=`
    <i class="ti ti-device-remote-control" style="color:var(--accent2);font-size:20px;flex-shrink:0;"></i>
    <div style="flex:1;">
    ...
    <button class="btn btn-sm" onclick="importIRDBRemote('${escAttr(file.path)}','${escAttr(name)}')"><i class="ti ti-download"></i> Import</button>`;
  el.appendChild(hdr);
  if (!buttons.length) {
    el.innerHTML+=`<div class="empty-state"><i class="ti ti-file-x"></i><p>No compatible NEC buttons in this file.</p></div>`;
    return;
  }
  buttons.forEach(btn => {
    const box=document.createElement('div');
    ...
    box.innerHTML=`
      ...
      <div class="signal-meta" style="margin-bottom:10px;">
        <span class="signal-chip addr"><i class="ti ti-map-pin" style="font-size:10px;"></i> ${escHtml(btn.addr||'—')}</span>
        <span class="signal-chip cmd"><i class="ti ti-terminal" style="font-size:10px;"></i> ${escHtml(btn.cmd||'—')}</span>
        ${btn.proto?`<span class="signal-chip proto">${escHtml(btn.proto)}</span>`:''}
      </div>
      <button class="btn btn-green btn-sm" onclick="sendButtonSignal('${escAttr(btn.addr)}','${escAttr(btn.cmd)}')" ${writer?'':'disabled'}><i class="ti ti-send"></i> Send</button>`;
    el.appendChild(box);
...
async function importIRDBRemote(path, name) {
  try {
    const resp=await fetch(`https://raw.githubusercontent.com/Lucaslhm/Flipper-IRDB/main/${path}`);
    const text=await resp.text();
    const buttons=parseIRFile(text);
    if (!buttons.length) { alert('No compatible buttons found in this remote.'); return; }
    // Mirror folder structure
    const parts=path.split('/'); let parentId=null;
    for(let i=0;i<parts.length-1;i++) {
      const fn=parts[i];
      let folder=library.find(f=>f.type==='folder'&&f.name===fn&&f.parentId===parentId);
      if(!folder) { const fid=genId(); library.push({type:'folder',id:fid,name:fn,parentId,open:true}); parentId=fid; }
    ...
    btn.innerHTML = '<i class="ti ti-check"></i> Imported!';
    btn.disabled = true;
